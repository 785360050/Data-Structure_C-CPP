...
namespace Painter
{
	namespace Tree
	{
		...
		struct Coordinate
		{
			...
			QRectF Get_Area_Size()
			{
				double height=_Get_Height();
				return {QPointF{-width/2,-height/2},QSizeF{width,height}};
			}

			double _Get_Height()
			{
				int level=2*this->level-1;
				return level*unit.height();
			}
			...
			void Set_Level(int level)
			{
				...
				left_top.setX(-width/2+half.width());
				left_top.setY(-_Get_Height()/2);
		...
		public:
			void Update_Tree(const Serialized_Container<DataType, branch, NodeType> &container)
			{
				...
				bound=coordinate.Get_Area_Size();

			}
			// QRectF Get_Area_Size() {return coordinate.Get_Area_Size();}
			QRectF Get_Area_Size() {return bound;}
		...
		template <typename DataType, int branch, typename NodeType = TreeNode<DataType>>
		class Base : public Painter
		{
		protected:
			// Adapter::Tree_Binary_Search<int,int> tree;
			Tree::Serialized_Container<DataType,branch,NodeType> container;
			Tree::Drawer<DataType,branch,NodeType> drawer;
