...
#include "Liner_List_Link/List_Head.h"
#include <memory>
...
void Test_SeqList()
{
	...
	std::cout << "获取第二个元素值:" << seq_list->List_GetElement(2) << std::endl;
...
{
	//std::unique_ptr<Liner_List<int, int>> seq_list = std::make_unique<Sequential_List_Array<int>>(5);
	Liner_List<List_Node_SingleWay<int>, int>* seq_list = new Link_List_Forward<int>;
	seq_list->List_Show("Initialize");
	...
	seq_list->Element_Insert(2, 9);
	seq_list->List_Show("第二个位置插入9后，链表扩展一个元素");
