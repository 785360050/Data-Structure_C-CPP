...
public:
	...
	//判断线性表是否为空
	virtual bool List_CheckEmpty() const { return length == 0; }
	//求当前表长度
...
public:
	...
	//定位并返回单链表第pos个元素节点
public:///元素操作
	List_Node_SingleWay<DataType>* Element_Locate(int pos) override
	{
		try
		{//判断非空且不超过l->length
			if (pos <= 0)
...
public:
	...
	//定位并返回单链表第pos个元素节点
public:///元素操作
	List_Node_DoubleWay<DataType>* Element_Locate(int pos) override
	{
		try
		{//判断非空且不超过l->length
			if (pos <= 0)
