	...
	std::pair<int, B_Node*> split(B_Node* node, int mid)
{
	...
	B_Node* sp = B_Tree_Create_Node(order);	///新建同层分裂节点
...
public:
	...
	void B_Tree_Insert(int key)
	{
		...
		{///插入目标节点不存在时(即空树时)
			B_Node* r = B_Tree_Create_Node(order);///创建上层节点
			root = r;
			r->key[0] = key;
			++r->length;
		}
		if (root_new)
		{
			B_Node* root = B_Tree_Create_Node(order);///创建上层节点
			this->root = root;
