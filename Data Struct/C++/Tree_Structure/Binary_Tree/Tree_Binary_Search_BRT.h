...
public:
	Node_Binary_Search_RB(std::string name, DataType element=0,Colour colour=red)
		:name(name),element(element),left(nullptr),right(nullptr), colour(colour), parent(nullptr) {};
...
public:
	...
	int count;
public:
	Tree_Binary_Search_RBT(Node_Binary_Search_RB<DataType>* node = nullptr) :root(node), count(0) {};
...
private:
	...
	void RBTree_Visit(const Node_Binary_Search_RB<DataType>* const node)
	{
		if (node)
