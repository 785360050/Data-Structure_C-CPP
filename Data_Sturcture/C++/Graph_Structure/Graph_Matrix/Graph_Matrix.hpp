...
public:
	Graph_Matrix(bool directed, int num_vertex) : Graph_Structure<Vertex_Matrix<DataType>, DataType>(directed, num_vertex),
...
namespace Storage
{
	// 邻接矩阵
	template <typename DataType, typename ElementType = Vertex_Matrix<DataType>>
	class Graph_Matrix: public Graph_Structure<ElementType, DataType>
	{ /// 边权值<0表示∞，=0表示不存在,无向图=1表示存在
	private:
		bool *state_visited{new bool[num_vertex]{}};								// 访问状态
		Vertex_Matrix<DataType> *vertex{new Vertex_Matrix<DataType>[num_vertex]{}}; // 存放顶点的集合
		int **edge{};						 // 邻接矩阵
		// std::set<ElementType> vertex;
	...
	public:
		...
		void Edge_Add(int no_origin, int no_destination, int weight) override
		{
			try
			{
				if (no_origin < 0 || no_origin >= this->num_vertex)
		...
		void Edge_Delete(int no_origin, int no_destination)
		{
			try
			{
				if (no_origin < 0 || no_origin >= this->num_vertex)
