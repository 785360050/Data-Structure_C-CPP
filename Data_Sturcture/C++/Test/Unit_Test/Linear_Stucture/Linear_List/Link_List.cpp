...
BOOST_AUTO_TEST_CASE(Operations_Sequence)
{

    Link_List_Static<int,5> list;
    BOOST_CHECK_THROW(list.Element_Insert(0, 0), std::out_of_range);
    BOOST_CHECK_THROW(list.Element_Insert(2, 2), std::out_of_range);
    ...
    BOOST_CHECK_THROW(list[0], std::out_of_range);
    BOOST_CHECK_THROW(list[6], std::out_of_range);
    BOOST_CHECK_THROW(list.Element_Insert(1, 1);, std::runtime_error);
...
BOOST_AUTO_TEST_CASE(Operations_Dynamic_Element)
{
    ...
    {
        Link_List_Static<Element<int>, 5> list;
        BOOST_CHECK_THROW(list.Element_Insert(0, {0,0}), std::out_of_range);
        BOOST_CHECK_THROW(list.Element_Insert(2, {2,2}), std::out_of_range);
        ...
        BOOST_CHECK(list.Get_Size() == 5);
        for (int i = 1; i <= 5; i++)
            BOOST_CHECK(list[i] == Element<int>(i, 10 + i));
        BOOST_CHECK_THROW(list[0], std::out_of_range);
        BOOST_CHECK_THROW(list[6], std::out_of_range);
        BOOST_CHECK_THROW(list.Element_Insert(1, {1,1});, std::runtime_error);

        list.Element_Update(1, {2,22});
        BOOST_CHECK(list.Get_Size() == 5);
        BOOST_CHECK(list[1] == Element<int>(2,22));
        list.Element_Delete(1);
        BOOST_CHECK(list.Get_Size() == 4);
