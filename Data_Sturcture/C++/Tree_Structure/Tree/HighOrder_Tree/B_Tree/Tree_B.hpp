...
private:
	...
	//定位key在节点中的位置
	int B_Tree_Node_Locate(NodeType* node, int key)
	...
	std::pair<int, NodeType*> split(NodeType* node, int mid)
{
	/// <summary>
	...
	NodeType* sp = B_Tree_Create_Node(order);	///新建同层分裂节点

	//int guard = node->length == node->order ? node->length+1 : node->length;
	...
	sp->length = order - mid - 1;	///一半分裂出去，非奇数node->order时可能有bug
	...
	return std::pair<int, NodeType*>(element, sp);;
}
	///索引指针 与 元素下标 互转
	int position_to_index_left(int position)
	{
		return position;
	}
	int position_to_index_right(int position)
	{
		return position + 1;
	}
	int index_left_to_position(int position)
	{
		return position;
	}
	int index_right_to_position(int position)
	{
		return position - 1;
	}
	//B树调整
...
public:
	//创建有order个分叉的节点
	NodeType* B_Tree_Create_Node(int order)
	{
		return new NodeType(order);
	}
