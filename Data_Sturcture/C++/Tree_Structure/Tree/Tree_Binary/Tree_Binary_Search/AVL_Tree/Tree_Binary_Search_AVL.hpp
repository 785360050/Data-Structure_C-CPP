...
namespace Storage
{
	template <typename DataType, typename KeyType = int, ADT::Binary_Node NodeType = Node_Binary_Search_Balance<DataType, KeyType>>
	class Tree_Binary_Search_AVL : public Logic::Tree_Binary_Search<DataType, KeyType, NodeType>
	{
	public:
		Tree_Binary_Search_AVL() : Logic::Tree_Binary_Search<DataType, KeyType, NodeType>() {};
		~Tree_Binary_Search_AVL() = default;
	...
	private:
		...
		NodeType *Node_Delete(NodeType *node, KeyType key)
		{
			...
			{ /// 找到目标节点，删除并调整
				...
				{													 /// 删除节点左右孩子都存在,用逻辑前驱代替
					temp = this->Node_Precursor(node);				 // 找到逻辑前驱
					node->replace_by(temp);							 // 用逻辑前驱替换
					node->left = Node_Delete(node->left, temp->key); // 替换后删除替换的节点
				}
			}
	...
	public:
		// 插入AVL节点，在平衡二叉树tree中，双亲为parenr的[左/右]位置,递归过程中同步实现平衡化
		void Element_Insert(KeyType key, DataType element = DataType{})
		{
			this->root = Node_Insert(this->root, key, element);
		}
