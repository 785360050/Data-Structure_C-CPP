...
#include <vector>
template <typename Binary_Heap, typename ElementType>
void Check(Binary_Heap heap)
{
    std::vector<ElementType> v;
    v.resize(10, ElementType{});
    while (!heap.Is_Empty())
    {
...
BOOST_AUTO_TEST_CASE(Operations)
{
    ...
    // BOOST_CHECK(heap.Get_Capacity() == 5);
    ...
    Check<decltype(heap_min), int>(heap_min);
    ...
    Check<decltype(heap_max), int>(heap_max);
