...
namespace Storage
{
	...
	class Sequential_List : public Logic::Linear_List<ElementType>
	{
	protected:
	...
	public:
		Sequential_List(size_t size, ElementType *storage, size_t capcity)
			: Logic::Linear_List<ElementType>(size), storage{storage}, capcity{capcity} {};
	...
	public:
		...
		virtual void List_Show(const string &string)
		{
			std::cout << string << std::endl
					  ...
					  << "storage->";
			for (size_t index = 0; index < capcity; index++)
...
#include "../ADT.hpp"
static_assert(ADT::Linear_List<Sequential_List_Static<int, 5>, int>);
static_assert(ADT::Linear_List<Sequential_List_Dynamic<int>, int>);
#endif